...
function ProjectModal({ project, onClose }) {
    ...
    return (
        ...
        >
            <div className={`modal${closing ? ' closing' : ''}`}> {/* Utilisez la classe 'closing' pour déclencher l'animation */}
                <h1 className="modal-title">{project.title}</h1>
                <div className='modal-project'>
                    <Carrousel slides={project.slides} />
                    <div className='modal-container-info'>
                        <p className="modal-description"> <strong>Contexte : </strong>{project.presentation}</p>
                        <p className='modal-description'><strong>Acquis : </strong>{project.acquired}</p>
                ...
                <SvgIcon component={CloseIcon} className='modal-close' onClick={handleClose} />
            </div>
        </Modal>
    );
}
